...
function api(action,extra={}){
  return new Promise((resolve,reject)=>{
    const callback='shiftSwap_'+Date.now()+'_'+Math.random().toString(36).slice(2);
    ...
    window[callback]=function(data){
      ...
      finished=true;
      cleanup();
      if(data && data.ok===false) reject(new Error(data.error||'Google Sheets error.'));
      else resolve(data);
    };
    ...
    script.onerror=function(){
      ...
      finished=true;
      cleanup();
      reject(new Error('Google Sheets connection failed. Check the Apps Script Web App deployment.'));
    };
...
async function load(){
  try{
    ...
    if(!r || !r.ok)
      throw Error((r&&r.error)||'Unable to load ShiftSwap data.');
...
$('postForm').onsubmit=async e=>{e.preventDefault();try{let name=$('name').value.trim();localStorage.ssname=name;let r=await api('addDay',{name,date:$('date').value,note:$('note').value.trim()});if(!r.ok)throw Error(r.error);e.target.reset();$('modal').classList.add('hidden');toast('Saved to Google Sheets.');await load()}catch(x){alert(x.message)}};
async function pick(id){let name=prompt('Enter your name to pick up this day:',localStorage.ssname||'');if(!name)return;try{let r=await api('pickDay',{id,name:name.trim()});if(!r.ok)throw Error(r.error);localStorage.ssname=name.trim();toast('Day picked up.');await load()}catch(x){alert(x.message)}}window.pick=pick;
...
$('msgForm').onsubmit=async e=>{e.preventDefault();try{let r=await api('addMessage',{name:$('msgName').value.trim(),message:$('msgText').value.trim()});if(!r.ok)throw Error(r.error);e.target.reset();toast('Message saved.');await load()}catch(x){alert(x.message)}};
