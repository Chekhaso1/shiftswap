...
$('gateForm').onsubmit=e=>{
  ...
  if($('code').value.trim().toUpperCase()===ACCESS_CODE){
    sessionStorage.ok='1';
    unlock();
  }else $('err').textContent='Incorrect access code.';
};

/* Google Apps Script connection.
   Uses JSONP instead of fetch(), avoiding the GitHub Pages -> Apps Script
   cross-origin "Failed to fetch" problem. */
function api(action,extra={}){
  return new Promise((resolve,reject)=>{
    const callback='shiftSwapCallback_'+Date.now()+'_'+Math.random().toString(36).slice(2);
...
$('postForm').onsubmit=async e=>{
  ...
  try{
    ...
    if(!$('date').value)throw Error('Please select a date.');

    localStorage.ssname=name;

    let r=await api('addDay',{
      name:name,
      date:$('date').value,
    ...
    // Reload the board after the save completes.
    await load();
  }catch(x){
    alert(x.message);
  }
...
async function pick(id){
  let name=prompt('Enter your name to pick up this day:',localStorage.ssname||'');
  if(!name)return;
  ...
  try{
    let r=await api('pickDay',{id:id,name:name});
    ...
    // Reload immediately so every board section reflects the change.
    await load();
  }catch(x){
    alert(x.message);
  }
...
function card(x){
  return`<article class="item ${x.status==='taken'?'taken':''}">
    ...
    <div>
      ${x.status==='taken'?'<span class=badge>TAKEN</span>':`<span class=badge>AVAILABLE</span><br><button onclick="pick('${x.id}')">Pick Up Day</button>`}
    </div>
  </article>`;
}
...
function render(){
  ...
  let a=days.filter(x=>JSON.stringify(x).toLowerCase().includes(q));
  let av=a.filter(x=>x.status==='available');
  let tk=a.filter(x=>x.status==='taken');
...
function renderCalendar(){
  let y=month.getFullYear(),m=month.getMonth();
  ...
  for(let i=0;i<42;i++){
    let n=i-off+1;
    let d=n<1?new Date(y,m-1,new Date(y,m,0).getDate()+n):
      n>last?new Date(y,m+1,n-last):
      new Date(y,m,n);
...
$('msgForm').onsubmit=async e=>{
  ...
  try{
    ...
    if(!name)throw Error('Please enter your name.');
    if(!message)throw Error('Please enter a message.');

    let r=await api('addMessage',{name:name,message:message});
